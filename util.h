...
typedef struct CPU_State_Struct {
	...
	uint32_t IF_ID_NPC; // the pc value that gets us this inst
	uint32_t IF_ID_stall_or_no_inst; // whether the instruction needs to be executed or not
	...
	uint32_t ID_EX_REG2;
	int ID_EX_IMM;
	uint32_t ID_EX_ZERO_EXT_IMM;
	unsigned char ID_EX_DEST_REG;
	uint32_t ID_EX_stall_or_no_inst;

	//EX_MEM_latch
	instruction* EX_MEM_INST;
	uint32_t EX_MEM_NPC;
	uint32_t EX_MEM_ALU_OUT;
	uint32_t EX_MEM_WRITE_VALUE;
	uint32_t EX_MEM_BR_TARGET;
	uint32_t EX_MEM_BR_TAKE;
	unsigned char EX_MEM_DEST_REG;
	uint32_t EX_MEM_stall_or_no_inst;

	//MEM_WB_latch
	instruction* MEM_WB_INST;
	uint32_t MEM_WB_NPC;
	uint32_t MEM_WB_ALU_OUT;
	uint32_t MEM_WB_MEM_OUT;
	uint32_t MEM_WB_BR_TAKE; // what is the need of this?????
	unsigned char MEM_WB_DEST;
	uint32_t MEM_WB_stall_or_no_inst;

	//Forwarding
	//unsigned char EX_MEM_FORWARD_REG;
	//unsigned char MEM_WB_FORWARD_REG;
	uint32_t EX_MEM_ALU_FORWARD_VALUE;
	uint32_t MEM_WB_ALU_FORWARD_VALUE;
	uint32_t MEM_WB_to_MEM_DATA_FORWARD_VALUE;

	unsigned char first_alu_source;
	unsigned char second_alu_source;
	unsigned char write_data_mem_source;

	//To choose right PC
	uint32_t IF_PC;
	uint32_t JUMP_PC;
	uint32_t BRANCH_PC;
	unsigned char pc_source;
} CPU_State;
